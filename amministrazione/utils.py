...
def strfdelta_round(tdelta, round_period='second'):
    ...
    attend period from days down to smaller period, round to minimum period
    omit zero value period
    """
    period_names = ('hour', 'minute', 'second', 'millisecond')
    if round_period not in period_names:
        raise Exception(f'round_period "{round_period}" invalid, should be one of {",".join(period_names)}')
    period_seconds = (3600, 60, 1, 1 / pow(10, 3))
    period_desc = ('ore', 'min', 'secs', 'msecs')
